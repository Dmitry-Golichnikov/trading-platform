"""
ExtraTrees модель для классификации и регрессии.

Обёртка над sklearn ExtraTrees с поддержкой:
- Feature importances
...
@ModelRegistry.register(
    "extra_trees",
    description="Extra Trees ensemble model",
    tags=["tree-based", "ensemble", "sklearn"],
)
class ExtraTreesModel(BaseModel, ClassifierMixin, RegressorMixin):
    """
    ExtraTrees обёртка для классификации и регрессии.
    ...
    Args:
        ...
        **hyperparams: Гиперпараметры ExtraTrees
    """

    def __init__(self, task: str = "classification", **hyperparams):
        """
        Инициализация ExtraTrees модели.

        Args:
            task: Тип задачи ('classification' или 'regression')
            **hyperparams: Гиперпараметры для ExtraTrees
        """
    ...
    def _set_default_params(self) -> None:
        ...
        defaults = {
            ...
            "min_samples_leaf": 1,
            "max_features": "sqrt",
            "bootstrap": False,  # ExtraTrees по умолчанию не использует bootstrap
            "random_state": 42,
            "n_jobs": -1,
            "verbose": 0,
        }
    ...
    def fit(
        ...
        **kwargs,
        ...
        logger.info(f"Начало обучения ExtraTrees модели (задача: {self.task})")
    ...
    def __repr__(self) -> str:
        ...
        return f"ExtraTreesModel(task={self.task}, n_estimators={n_estimators}, {fitted_status})"
