...
class LabelingSetInfo(BaseModel):
    """Labeling set information"""
...
class LabelingTaskCreateRequest(BaseModel):
    """Labeling task creation request"""

    name: str = Field(..., description="Logical name of the labeling set")
    dataset_ids: Optional[List[str]] = Field(None, description="Specific dataset IDs to process")
    feature_set_id: Optional[str] = Field(None, description="Optional feature set ID to link")
    apply_to_all: bool = Field(False, description="Label all available datasets")
    method: LabelingMethod = Field(..., description="Labeling method")
    config: Dict[str, Any] = Field(..., description="Labeling configuration payload")
    description: Optional[str] = Field(None, description="Notes or rationale")
    auto_start: bool = Field(True, description="Start tasks immediately after creation")


class LabelingTaskInfo(BaseModel):
    """Labeling task information"""
...
class LabelingTaskActionResponse(BaseModel):
    """Response returned after labeling task action"""

    task: LabelingTaskInfo
    message: str
