...
class DatasetService:
    ...
    def get_dataset_data(
        ...
        frames = []
        for file in files:
            df_part = self._read_parquet_file(file)
    ...
    def _read_parquet_file(self, file: Path) -> Optional[pd.DataFrame]:
        """Read parquet file with fallback to fastparquet if installed."""
        try:
            ...
            try:
                ...
                print(f"Fastparquet fallback failed for {file}: {exc2}")
                return None
