...
// Labeling types
export interface LabelingSetInfo {
  id: string;
  name: string;
  dataset_id: string;
  method: 'horizon' | 'triple_barrier' | 'custom' | 'regression';
  config: Record<string, any>;
  num_samples: number;
...
export type LabelingTaskStatus = 'queued' | 'running' | 'paused' | 'completed' | 'failed' | 'cancelled';

export interface LabelingTaskInfo {
  id: string;
  name: string;
  dataset_id: string;
  labeling_set_id: string;
...
export interface LabelingTaskCreatePayload {
  name: string;
  dataset_ids?: string[];
  feature_set_id?: string;
  apply_to_all?: boolean;
