...
### Источники данных:
- Модуль данных (Этап 01) загружает исторические котировки через REST метод `getHistory` Tinkoff Invest API ([документация](https://tinkoff.github.io/investAPI/get_history/)).
- Метод возвращает годовые архивы минутных свечей и ограничен 30 unary-запросами в минуту. Поэтому при массовом расчёте индикаторов следует планировать батчинг, кеширование и уважительное использование кэшей артефактов,
  чтобы не превысить лимиты и не блокировать пайплайны расчёта признаков.
- В тестах и примерах рекомендуется использовать заранее подготовленные локальные архивы, чтобы не зависеть от ограничений API во время CI.
- Обработанные данные доступны в `artifacts/data/{TICKER}/{TIMEFRAME}/{YEAR}.parquet`, где доступны таймфреймы 1m/5m/15m/1h/4h/1d; индикаторы должны уметь работать с любым из них.
